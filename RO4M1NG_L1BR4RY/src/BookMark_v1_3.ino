...
void handleNodeFiles() {
    ...
    if (section == "") {
        ...
        if (isLocal) {
            ...
            if (dir) {
                ...
                if (totalFiles == 0) {
                    ...
                    server.sendContent(String(F("<script>document.querySelector('.section-title').innerHTML += ' [")) +
                                     String(fileCount) +
                                     F(" F1L35]';</script>"));
                }
        ...
        server.sendContent(String(F("<a href='/node-files?node=")) + nodeSSID +
...
void setup() {
    ...
    if (!SD.begin(SD_CS_PIN, SPI, 40000000)) {
        Serial.println("Wrew FAILED!");
        Serial.println("ESP-32: SD card initialization failed. Check wiring and card.");
        for (int i = 0; i < 5; i++) {
            digitalWrite(ledPin, HIGH);
            delay(100);
            digitalWrite(ledPin, LOW);
    ...
    if (!SD.begin(SD_CS_PIN)) {
        ...
        Serial.println("ESP12: SD card initialization failed. Check wiring and card.");
        for (int i = 0; i < 5; i++) {
            digitalWrite(ledPin, HIGH);
            delay(100);
            digitalWrite(ledPin, LOW);
